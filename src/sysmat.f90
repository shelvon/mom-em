...
CONTAINS
  ...
  SUBROUTINE sysmat_mueller(domain, ga, media, zwl, A, N)
    ...
    INTEGER, INTENT(IN)                           :: N
    COMPLEX(KIND=dp), DIMENSION(2*N,2*N), INTENT(INOUT) :: A

    COMPLEX(KIND=dp), DIMENSION(:,:,:), ALLOCATABLE     :: nxD, nxHline, nxK
    COMPLEX(KIND=dp), DIMENSION(N,N)  :: nxD1, nxD2, nxHline1,  &
                                         nxHline2, nxK1, nxK2
    INTEGER                 :: nd, nind, idom, ndom, imat
    ...
    ndom = SIZE(domain(0:))
    ALLOCATE( nxD(N,N,0:ndom-1), nxHline(N,N,0:ndom-1), nxK(N,N,0:ndom-1) )
    ALLOCATE( ri(0:ndom-1), eps(0:ndom-1), mu(0:ndom-1) )
    ...
    DO idom = 0, 1
      imat = domain(idom)%media ! media index for the domain
...
!        ' - Relative index (n) of medium ', idom, ': ', ri(idom)
!      WRITE(*,'(A,I0,A,'//fmt_cmplx//'SS)') &
!        ' - Relative permittivity (epsilon) of medium ', idom, ': ', eps(idom)/eps0
!      WRITE(*,'(A,I0,A,'//fmt_cmplx//'SS)') &
!        ' - Relative permeability (mu) of medium ', idom, ': ', mu(idom)/mu0
    ...
    A(1:N,1:N) = A(1:N,1:N) - nxK(:,:,1)*2.0_dp*mu(1)/(mu(0)+mu(1))

    A(1:N,(N+1):(2*N)) = A(1:N,(N+1):(2*N))  &
                       -(nxD(:,:,0)+nxHline(:,:,0))*2.0_dp*eps(0)/(mu(0)+mu(1))
    A(1:N,(N+1):(2*N)) = A(1:N,(N+1):(2*N))  &
                       +(nxD(:,:,1)+nxHline(:,:,1))*2.0_dp*eps(1)/(mu(0)+mu(1))

    A((N+1):(2*N),1:N) = A((N+1):(2*N),1:N)  &
                       +(nxD(:,:,0)+nxHline(:,:,0))*2.0_dp*eps(0)/(eps(0)+eps(1))
    A((N+1):(2*N),1:N) = A((N+1):(2*N),1:N)  &
                       -(nxD(:,:,1)+nxHline(:,:,1))*2.0_dp*eps(1)/(eps(0)+eps(1))

    A((N+1):(2*N),(N+1):(2*N)) = A((N+1):(2*N),(N+1):(2*N))  &
                               + nxK(:,:,0)*2.0_dp*eps(0)/(eps(0)+eps(1))
    A((N+1):(2*N),(N+1):(2*N)) = A((N+1):(2*N),(N+1):(2*N))  &
                               - nxK(:,:,1)*2.0_dp*eps(1)/(eps(0)+eps(1))

  END SUBROUTINE sysmat_mueller

  ! Computes the system matrix for eigen modes problem of single particle
  ! described by struct b and for wavelength of given index wlind.
  SUBROUTINE sysmat_mueller_b(b, wlind, A, N)
    TYPE(batch), INTENT(IN) :: b
    INTEGER, INTENT(IN) :: wlind
    INTEGER, INTENT(IN) :: N
...
!    !$OMP END PARALLEL
  ...
  END SUBROUTINE sysmat_mueller_b
  ...
  SUBROUTINE computenxD(omega, ri, mesh, ga, prd, qd, nxD)
    ...
    COMPLEX(KIND=dp), DIMENSION(mesh%nedges,mesh%nedges), INTENT(INOUT) :: nxD
  ...
  ! H_mng = -sum_(+-) int_(Tm+-) dC m(r).fm(r)
  !         x int_Sn' dS' div'_S(fn(r'))*O'_gG(r,r'),
  ! where m(r) is a unit vector tangent to close line C which has
  ! induced orientation from that of S.
  SUBROUTINE computenxHline(omega, ri, mesh, ga, prd, qd, nxHline)
    ...
    TYPE(quad_data), INTENT(IN)       :: qd
    ! Input and output variables
    COMPLEX(KIND=dp), DIMENSION(mesh%nedges,mesh%nedges), INTENT(INOUT) :: nxHline
...
!      !$OMP PARALLEL DEFAULT(NONE)&
!      !$OMP SHARED(intnxHaux,ptv,mesh,k,ga,prd,m,qd)&
      ...
      DO n=1,mesh%nfaces
        ...
        DO t=1,3
          ...
          DO r=1,nweights
            ...
            intnxHaux(:,r+nweights*(t-1),n) = &
              intK1Mueller(ptv(:,r+nweights*(t-1)), n, mesh, k, ga, prd, near, qd)
          END DO
        END DO
!      END DO
!      !$OMP END DO
...
!      DO n=1,mesh%nfaces
  ...
  SUBROUTINE computenxK(omega, ri, mesh, ga, prd, qd, nxK)
    ...
    TYPE(quad_data), INTENT(IN)       :: qd
    ! Input and output variables
    COMPLEX(KIND=dp), DIMENSION(mesh%nedges,mesh%nedges), INTENT(INOUT) :: nxK
  ...
  ! Compute nxD, nxHline, nxK together.
  ! D_mng = int_Sm dS fm(r).n(r) x int_Sn' dS' J_g*fn(r')*O'_gG(r,r'),
  ! where "x" means cross product and "." means dot product.
  !
  ! H_mng = int_Sm dS fm(r).n(r) x grad int_Sn' dS' div'_S(fn(r'))*O'_gG(r,r'),
  ...
  ! K_mng = int_Sm dS fm(r).n(r)x int_Sn' dS' [O'_g grad'G(r,r')]x(J_g*fn(r')),
  ! where "x" means cross product and "." means dot product.
  SUBROUTINE compute_nxDHK(omega, ri, mesh, ga, prd, qd, nxD, nxHline, nxK)
    IMPLICIT NONE
    ! Input variables
    COMPLEX(KIND=dp), INTENT(IN)      :: omega
    COMPLEX(KIND=dp), INTENT(IN)      :: ri
    ...
    ! Internal variables
    COMPLEX(KIND=dp)    :: k, eps, mu, coeffD, coeffH, intnxD, intnxK
    INTEGER             :: n, m, p, q, t, r, index1, index2, nweights
    INTEGER, PARAMETER  :: nweightsH = 6
    REAL(KIND=dp)       :: Am
    LOGICAL             :: near
    REAL(KIND=dp), DIMENSION(3, qd%num_nodes)   :: qpm
    REAL(KIND=dp), DIMENSION(3)                 :: nor, diff
    REAL(KIND=dp), DIMENSION(3)                 :: p1, p2, ptm
    REAL(KIND=dp), DIMENSION(3,3)               :: tv
    REAL(KIND=dp), DIMENSION(3,nweightsH*3)     :: ptv
    REAL(KIND=dp), DIMENSION(nweightsH)         :: glp, glw
    REAL(KIND=dp), DIMENSION(3,nweightsH*3,3)   :: fmvH
    REAL(KIND=dp), DIMENSION(3,qd%num_nodes,3)  :: fmv
    COMPLEX(KIND=dp), DIMENSION(3)              :: intnxH
    COMPLEX(KIND=dp), DIMENSION(3,3,qd%num_nodes,mesh%nfaces) :: intnxDaux
    COMPLEX(KIND=dp), DIMENSION(3,3,qd%num_nodes,mesh%nfaces) :: intnxKaux
    COMPLEX(KIND=dp), DIMENSION(3,nweightsH*3,mesh%nfaces) :: intnxHaux

    WRITE(*,*) 'Building the nxD, nxHline and nxK matrices for mode solver.'
    ...
    DO m=1,mesh%nfaces
      ...
      DO p=1,3
        ...
        ! output: f_n(r), but for n(r) x H(fn(r)) calculation
        ! and a different quadrature is used.
        CALL vrwg(ptv(:,:),m,p,mesh,fmvH(:,:,p))
      END DO

      !$OMP PARALLEL DEFAULT(NONE)&
      !$OMP SHARED(nweights,intnxDaux,intnxKaux,qpm,mesh,k,ga,prd,m,qd)&
      !$OMP PRIVATE(n,near,r)
      !$OMP DO SCHEDULE(STATIC)
      DO n=1,mesh%nfaces
        near = near_faces(mesh, prd, n, m, ga)
      ...
      DO n=1,mesh%nfaces
        ...
        DO t=1,3
          ...
          DO r=1,nweightsH
            ! intnxHaux = int_Sn' dS' div'_S(fn(r'))*O'_gG(r,r'), where the singularity
            ! is cancelled out in the combined kernel of G(r,r') in Mueller formulation.
            intnxHaux(:,r+nweightsH*(t-1),n) =  &
              intK1Mueller(ptv(:,r+nweightsH*(t-1)), n, mesh, k, ga, prd, near, qd)
          END DO
        END DO
        ...
        DO q=1,3
          ...
          DO t=1,3
            ...
            DO r=1,nweightsH
              ! Edges of the test triangle
              DO p=1,3
                intnxH(p) = intnxH(p) + glw(r)*dotr(tv(:,t),  &
                  fmvH(:,r+nweightsH*(t-1),p))*intnxHaux(q,r+nweightsH*(t-1),n)
              END DO
            END DO
          END DO
          ...
          DO p=1,3
            ...
            DO r=1,nweights
              ! int_Sm dS fm(r).n(r) x intnxDaux.
              intnxD = intnxD + qd%weights(r)*&
                      dotc( CMPLX(fmv(:,r,p),KIND=dp),  &
                            crossc(CMPLX(nor,KIND=dp), intnxDaux(:,q,r,n)) )
              ! int_Sm dS fm(r).n(r) x intnxKaux.
              intnxK = intnxK + qd%weights(r)*&
                      dotc( CMPLX(fmv(:,r,p),KIND=dp),  &
                            crossc(CMPLX(nor,KIND=dp), intnxKaux(:,q,r,n)) )
            END DO!r
            intnxD = intnxD*Am
            intnxK = intnxK*Am
    ...
    nxD(:,:) = coeffD*nxD(:,:)
    nxHline(:,:) = coeffH*nxHline(:,:)
  END SUBROUTINE compute_nxDHK
