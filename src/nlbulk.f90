...
CONTAINS
  ...
  ! Computes the action on nonlinear bulk dipole polarization Pnlb.
  FUNCTION Pthg_bulk_ga(mesh, nedgestot, omegaff, ri, x, ga, nlb, r, qd) RESULT(Pnlb)
    TYPE(mesh_container), INTENT(IN) :: mesh
    REAL (KIND=dp), INTENT(IN) :: omegaff
    COMPLEX (KIND=dp), INTENT(IN) :: ri
    COMPLEX (KIND=dp), DIMENSION(:,:,:), INTENT(IN) :: x
    ...
    DO na=1,SIZE(ga)
       DO ns=1,SIZE(x,3)
          ...
          ! Compute thg polarization in crystal frame.
          ! chi3 is in 3*10 D-matrix form by using the Kleinman's symmetry
          Pnlb2 = eps0*MATMUL(nlb%chi3, (/e2(1)**3, e2(2)**3, e2(3)**3,&
               3.0_dp*e2(2)*e2(3)*e2(3), 3.0_dp*e2(3)*e2(2)*e2(2), 3.0_dp*e2(1)*e2(3)*e2(3),&
               3.0_dp*e2(1)*e2(1)*e2(3), 3.0_dp*e2(1)*e2(2)*e2(2), 3.0_dp*e2(1)*e2(1)*e2(2),&
               6.0_dp*e2(1)*e2(2)*e2(3)/))

          ! Map Pnlb to lab frame.
          Pnlb(:,na,ns) = MATMUL(nlb%T, Pnlb2)
       END DO
    END DO
  END FUNCTION Pthg_bulk_ga
  ...
  ! Computes the excitation vector for a third-order dipolar bulk source for
  ! all group representations.
  SUBROUTINE srcvec_thg_bulk(mesh, nedgestot, omegaff, riff, rith, xff, ga, nlb,&
       qd_tri, qd_tetra, src)
    TYPE(mesh_container), INTENT(IN) :: mesh
    INTEGER, INTENT(IN) :: nedgestot
    REAL (KIND=dp), INTENT(IN) :: omegaff
    COMPLEX (KIND=dp), INTENT(IN) :: riff, rith
    COMPLEX (KIND=dp), DIMENSION(:,:,:), INTENT(IN) :: xff
    TYPE(group_action), DIMENSION(:), INTENT(IN) :: ga
    TYPE(medium_nlb), INTENT(IN) :: nlb
    TYPE(quad_data), INTENT(IN) :: qd_tri, qd_tetra
    ...
    omegash = 3.0_dp*omegaff
    k = rith*omegash/c0

    c1 = (0,1)*omegash*mu0
    c2 = 1.0_dp/((0,1)*omegash*eps0*(rith**2))
    ...
    DO m=1,mesh%nsolids
       ...
       !$OMP DO SCHEDULE(STATIC)
       DO t=1,nweights
          Jb(:,t,:,:) = -(0,1)*omegash*Pthg_bulk_ga(mesh, nedgestot, omegaff,&
               riff, xff, ga, nlb, qp(:,t), qd_tri)
       END DO
       !$OMP END DO
       !$OMP END PARALLEL
    ...
    src(:,:,:) = src(:,:,:)/REAL(SIZE(ga))
  END SUBROUTINE srcvec_thg_bulk
