C     AUTHOR:           XIAORUN (SHELVON) ZANG
C     DATE:             20.08.2018
C     THIS IS BASED ON THE driver.f PROGRAM IN TOMS 794.
C     THIS IS INTENDED TO PROVIDE A MUCHER EASIER INTERFACE
C     (i.e. SUBROUTINE HANKEL_TRANSFORM) FOR EXTERNAL CALLER
      SUBROUTINE HANKEL_TRANSFORM(HANKEL_KERNEL,gNU,gXI,gXLIMIT,HT,IERR)
C      SUBROUTINE HANKEL_TRANSFORM(HANKEL_KERNEL,NU,XI,XLIMIT,HT,IERR)
C
...
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION DUMMY,HT,JNUXV,SLTN
      DOUBLE PRECISION NU,XI,XLIMIT
      DOUBLE PRECISION gNU,gXI,gXLIMIT
...
C     .. Common blocks ..
      ...
      nout = i1mach(2)
C     ..
C   10 WRITE(NOUT,FMT=9000)
C      WRITE(NOUT,FMT=9010)
C
C    -------------------------------------------------------------------
C
...
C     OPEN FILES
C
C      WRITE(NOUT,FMT=9020)
C      READ(NIN,FMT=*,ERR=20,END=20) FILEER
C      GO TO 30

   20 FILEER = 'hankel.err'
C   30 WRITE(NOUT,FMT=9030) FILEER
C      WRITE(NOUT,FMT=9040)
C      READ(NIN,FMT=9050,ERR=40,END=40) FILEOU
C      GO TO 50

   40 FILEOU = 'hankel.out'
C   50 WRITE(NOUT,FMT=9060) FILEOU
C     FILE *** FILEER *** STORES MESSAGES FROM *** PROGRAM HANKEL ***.
C     SEARCH FOR ERROR MESSAGES IN FILE *** FILEER *** !
C      OPEN (UNIT=LUERR,FILE=FILEER,STATUS='UNKNOWN',ACCESS='SEQUENTIAL',
C     +     ERR=90)
C     FILE *** FILEOU *** STORES RESULTS FROM *** PROGRAM HANKEL ***.
C      OPEN (UNIT=LUOUT,FILE=FILEOU,STATUS='UNKNOWN',ACCESS='SEQUENTIAL',
C     +     ERR=90)
C
C    -------------------------------------------------------------------
C
...
C   60 WRITE(NOUT,FMT=9070)
C      READ(NIN,FMT=*) IMODE
C      IF (IMODE.EQ.0) STOP
C     INSTEAD READ INPUT, WE SET IMODE TO 1 PERMANENTLY
      IMODE = 1
C
C     *** NU *** IS THE ORDER OF THE HANKEL TRANSFORM:
C      WRITE(NOUT,FMT=9080)
C      READ(NIN,FMT=*) NU
C     NU WILL BE ASSIGNED THROUGH INPUT PARAMETERS
C
C     SET PARAMETER *** XI ***:
C      WRITE(NOUT,FMT=9090)
C      READ(NIN,FMT=*) XI
C     XI WILL BE ASSIGNED THROUGH INPUT PARAMETERS
C
      XI = gXI
      NU = gNU
      XLIMIT = gXLIMIT
...
C    -------------------------------------------------------------------
C
C      WRITE(NOUT,FMT=9150)
C      READ(NIN,FMT=*) ICON
C      IF (ICON.EQ.1) GO TO 10
C
C      WRITE(NOUT,FMT=9160)
C      WRITE(NOUT,FMT=9170)
C
C    -------------------------------------------------------------------
C
C     ERROR HANDLING:
...
C      GO TO 100
C
C   90 WRITE(NOUT,FMT=9180) FILEER
C      IERR = -10
C      CALL ERRMSS(IERR)
C
C  100 STOP
 ...
 9180 FORMAT (/,' MESSAGE DHINIT: ERROR ON OPENING FILE:',/,A80)
      END
C      END SUBROUTINE HANKEL_TRANSFORM
...
C     .. External Subroutines ..
      EXTERNAL DHJNUX
C     THE INTEGRAND FUN(X) IN HANKEL TRANSFORM IS DEFINED IN
C     THE EXTERNEL CALLER TO HANKEL_TRANSFORM
      EXTERNAL HANKEL_KERNEL
C     ..
C     .. Intrinsic Functions ..
...
C     HERE YOU HAVE TO TYPE YOUR FUNCTION:
C
C     HERE, WE USE HANKEL_KERNEL FUNCTION IN HANKEL_TRANSFORM SUBROUTINE
C     THIS IS THE FUNCTION *** FUN(X) ***:
          IF (X.LE.XLIMIT) THEN
              Y = HANKEL_API_MP_HANKEL_KERNEL(X)
C              Y = 1.0D0
          ELSE IF (X.GT.XLIMIT) THEN
C             WE PAD ZEROS FOR X LARGER THAN XLIMIT
C             THIS ACTUALLY CONVERT THE INFINITE INTEGRAL TO FINITE ONE
              Y = 0.0D0
          END IF
C
C     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
C     .. Common blocks ..
      COMMON /ARGUME/XI,NU,XLIMIT
C      COMMON /ARGUME/gXI,gNU,gXLIMIT
      COMMON /INOUTE/LUIN,LUOUT,LUERR
...
C      DOUBLE PRECISION XDATMA,XDATMI,YDATMA,YDATMI
